...
interface ConfirmDuplicateProps {
  closeView: () => void;
  id: string;
  resendSignal: () => void;
}

const ConfirmDuplicate: React.FC<ConfirmDuplicateProps> = ({ closeView, id, resendSignal }) => {
  const [loading, setLoading] = useState<boolean>(false);

  const confirmDup = async () => {
    const data = { id };
    setLoading(true);
    const res = await AuthPosturl(Apis.prices.duplicate_price, data);
    setLoading(false);
    if (res.status === 200) {
      return resendSignal();
    } else {
